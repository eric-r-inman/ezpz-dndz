...
pub fn base_router(state: AppState) -> Router {
  ...
  // Merge the dice router INTO app_router (rather than into the
  // outer Router) so its three endpoints contribute to the OpenAPI
  // schema generated by `finish_api_with`.  Doing it the other way
  // round leaves `/api/dice/*` invisible to `/scalar`.
