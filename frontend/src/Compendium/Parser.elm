...
rare) false-negative case.

-}
autoFlagSpecialReactions : Compendium.Creature -> Compendium.Creature
autoFlagSpecialReactions c =
    if c.hasSpecialReactions then
        -- Respect a manually-set True (won't happen on a fresh
        -- paste, but keeps the function idempotent).
        c
