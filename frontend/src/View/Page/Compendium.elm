...
full stat block on the right. The right pane has an action bar
with the "Add to Encounter" handoff plus per-creature edit /
duplicate / delete.
...
immediately, but the main tab's copy won't reflect them until
a reload (no live sync today). "Add to encounter" targets this
tab's encounter, not the main tab's — so those flows are best
avoided here.
...
spawns one fresh instance; the page stays put across adds.
