...
- *Legendary Action available* — ~legendaryActionStrip~.  Lists every queue
  ...
  ~StatBlockShow~ to unfold the stat block under the creature's card.  Filters:
  not the currently-active creature (rule: LA fires on OTHER creatures'
...
name whose creature has left the queue.  The strips and lists that name a
creature from elsewhere fire ~StatBlockShow~, which also scrolls the card into
view; the card's own name fires ~StatBlockToggle~.
