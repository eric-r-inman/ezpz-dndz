...
- ~pendingControl : Maybe PendingControl~ — two-step confirmation state for the
  destructive Reset / Clear actions (~PendingReset~ / ~PendingClear~).  When
  ~Just~, the drawer shows a confirmation instead of whatever else it was
  holding, so a misclick can't drop combat state.
...
- ~xpScope : XpScope~ + ~xpFilterOpen : Bool~ — which creatures the XP total
  counts, and whether the drawer is showing the panel that picks.  The title bar
  keeps the readout; the scope is chosen from the Actions column.
...
Two surfaces — the dice roller and the compendium browser — sit *outside* the
ADT because their substate has to survive a close (dice history, compendium
browser cache + filter).  They expose ~open : Bool~ on their own substate and
live as =Model.dice : DiceUi= / =Model.compendium : CompendiumUi=.  The roller
being outside is what =Update.PanelDrawer.soleOpen= exists to reconcile — see
the ~tasks.org~ entry on folding the drawer's stragglers into the ADT.
...
state the green action button in the Actions column reads "Run Encounter" and
...
Load panels' "To Server" / "From Server" radios use a different shape — labels
rename to "To Browser" / "From Browser" anonymously (via ~saveToServerLabel
auth~), and the submit routes through =Update.Save= / =Update.Load='s
auth-branched mutators.
...
| ~/api/encounter/saves(/:name)*~   | ~encounter_saves~ + ~encounter_save_nodes~            | yes       | User-named encounter saves (Save / Load panel).  Opaque bodies, exact echo.  PUT supports ?overwrite=true; rename / delete endpoints sit alongside; semantic 409 / 404 / 400 errors. |
...
- *Save / Load* — each opens its panel in the drawer, carrying the Server /
  Device radio pair that picks the destination.

  - *Save* — =View.Panel.Save=.  Filename input, list of existing server saves
    with inline rename / overwrite / delete.  Server uploads PUT to
    ~/api/encounter/saves/:name~ (server returns 409 on a name collision,
    surfaced as an inline overwrite-confirm banner; ~?overwrite=true~ upserts on
    confirm).  Device downloads go through ~File.Download.string~.
  - *Load* — =View.Panel.Load= lists the server saves for click-to-load with
    confirm; the Device path fires ~File.Select.file~ directly.  Either replaces
    the live encounter and forces ~round = 0~ + ~activeName = ""~ so the GM
    lands in pre-combat mode.  Successful loads also populate ~savedSnapshot~ +
    ~savedAs~.

- *Reset* / *Clear* — staged through ~pendingControl~ so a misclick can't drop
  combat state.  The first click puts a confirmation in the drawer (Cancel /
  coloured Confirm); the second applies.
...
panels: filename input, list of existing snapshots, inline overwrite-confirm
...
it.  ~Model.xpFilterOpen~ holds the panel's open state; ~XpFilterToggle~ /
~XpFilterClose~ flip and clear it, and ~XpScopeSet~ deliberately leaves it open.
=Main.subscriptions= adds one sub while it is open — ~Browser.Events.onKeyDown
(escKey XpFilterClose)~.  A click-outside sub would fire on every click landing
in the queue beside the panel.
...
7. *View* — =View/Panel/Foo.elm= for a workspace surface, which is nearly all of
   ...
   page.  Entry point is ~view : Model -> Html Msg~ that pattern-matches
   ~model.surface~ and falls through to ~text ""~; the chrome comes from
   =View.Panel.view= or =View.Modal.view=, whose shared fields line up.
8. *Drawer registration* — a panel also needs its variant in
   ~Model.isDrawerSurface~ and in ~View.PanelDrawer.surfacePanel~.  Those two
   lists move together; the second is what actually renders it.  A panel whose
   state has to outlive a close stays off the ADT instead, branched in
   ~View.PanelDrawer.current~ and reconciled by ~Update.PanelDrawer.soleOpen~ —
   see the straggler entry in ~tasks.org~ before adding to that set.  A
   per-creature editor takes a third shape: its body lives in
   =View/Inline/Foo.elm= and =View.PanelDrawer='s ~editor~ helper supplies the
   chrome, so every editor's target strip and close behaviour stay identical.
