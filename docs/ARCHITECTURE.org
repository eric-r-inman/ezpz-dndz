...
CompendiumUi=; the roller carries its own ~open : Bool~, while the browser
renders as the standalone =/compendium= tab and so has no open flag at all.  The
roller being outside is what =Update.PanelDrawer.soleOpen= exists to reconcile —
see the ~tasks.org~ entry on folding the drawer's stragglers into the ADT.
