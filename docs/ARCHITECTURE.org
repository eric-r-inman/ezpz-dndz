...
with nobody active.  In that state the Actions column's green Turn button
dispatches ~EncounterRun~, which calls ~Encounter.run~ — it sets the active
creature to the queue head (highest initiative).  Once someone is active the
same button dispatches ~NextTurn~ instead.
...
- *Save / Load* — each opens its panel in the drawer, carrying the Server /
  ...
  - *Load* — =View.Panel.Load= lists the server saves for click-to-load with
    ...
    the live encounter and forces ~round = 1~ + ~activeName = ""~ so the GM
...
The Actions column's XP trigger carries the encounter's XP total.
...
7. *View* — =View/Panel/Foo.elm= for a workspace surface, which is nearly all of
   ...
   page.  Entry point is ~view : View.Panel.Collapse -> Model -> Html Msg~ for a
   panel (a modal's takes no ~Collapse~, having nothing to fold into) that
   ...
   up.  The ~Collapse~ is passed straight through to =View.Panel.view=, which
   owns the heading row and decides whether the body renders at all.  That row
   folds the panel when clicked, so a control placed in ~titleLead~ takes its
