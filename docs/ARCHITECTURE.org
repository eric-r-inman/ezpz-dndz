...
- =server/src/=
  - ~main.rs~ — ~#[foundation_main]~ entry point.  Loads the per-user stores,
    ...
    subtree, merges the auth + dice + compendium + encounter + per-user-store
  ...
  - ~config.rs~ — ~MergeConfig~-derived ~Config~ with three skip-field resolvers
    (~paths~ / ~oidc~ / ~compendium_claim_user~) handling cross-field rules
    MergeConfig can't model directly.  ~RuntimePaths::from_data_dir~ derives
    every store path from the single ~--data-dir~ — there are deliberately no
    per-store path overrides.
  ...
  - ~per_user_store.rs~ — the generic per-user opaque-JSON store: a
    ~PerUserStore~ (a feature label + ~Arc<JsonFileStore<HashMap<UserId,
    Value>>>~), a shared ~PerUserStoreError~ that carries the label so messages
    stay semantic, and a ~routers()~ function registering the authenticated
    GET/PUT pair for all five stores — lore groups, condition presets, Save
    Chain presets, treasure table, treasure profiles.  Each was born as its own
    copy-pasted module; the five differed only in label strings and route paths,
    so they now share this one implementation.
- =lib/src/=
...
| UI substate     | =Ui/{AbilitySave,Account,Compendium,Condition,CrCalculator,Dice,Duplicate,GroupEdit,HpChange,Initiative,Load,LoadCompendium,Login,LoreEdit,Memo,ModalChrome,Note,PlaceholderRename,QuickAdd,RandomEncounter,Save,SaveChain,SaveCompendium,Timer,Toast,Treasure,TreasureTable}.elm= |
| Update / shell  | =Update/Shell.elm= (URL routing, ~/me~, encounter persist response, settings + control-menu popovers, ~NoOp~), =Update/Tabs.elm= (cross-tab BroadcastChannel receive paths — ~encounterFromOtherTab~, ~panelShowFromOtherTab~, ~incomingPanelShow~) |
...
| Update / domain | =Update/{AbilitySave,Account,Condition,CrCalculator,DeathSave,Dice,Duplicate,Encounter,HpChange,Initiative,LegendaryPip,Load,LoadCompendium,LoreEdit,Memo,ModalChrome,Note,PlaceholderRename,Preferences,QuickAdd,RandomEncounter,Save,SaveChain,SaveCompendium,SpellList,Timer,Toast,Treasure,TreasureTable}.elm= |
| Update / split  | =Update/Compendium/{Add,AddGroup,Browser,Bulk,Edit,Group,Lore,Paste}.elm=                                                               |
| View / modals   | =View/Modal/{AbilitySave,Compendium,CompendiumEdit,CompendiumPaste,Condition,CrCalculator,Dice,Duplicate,GroupEdit,HpChange,Initiative,Load,LoadCompendium,LoreEdit,Memo,Note,QuickAdd,RandomEncounter,Save,SaveChain,SaveCompendium,SpellList,Timer,Treasure,TreasureTable}.elm= |
...
| View / shell    | =View/{About,Account,AnonymousBanner,AppBar,Audio,AuthGate,Card,Chip,EncounterBar,Footer,Login,Modal,PanelControls,PanelDetail,PhaseToggle,RollPopup,StatBlock,Toast,Tooltips,Workspace}.elm=.  =View/AuthGate.elm= holds the ~clickWhenAuthed~ / ~tooltipWhenAuthed~ helpers for sign-in-nudging server-only controls. |
...
| =lore-groups.json=, =condition-presets.json=, =save-chain-presets.json=, =treasure-table.json=, =treasure-profiles.json= | ~HashMap<UserId, Value>~ | The five ~PerUserStore~ files — opaque JSON payloads whose schema the Elm side owns; the server just round-trips them through a shared GET/PUT pair. |
...
| =lore-groups.json= / =condition-presets.json= / =save-chain-presets.json= / =treasure-table.json= / =treasure-profiles.json= | ~/api/{lore-groups,condition-presets,save-chain-presets,treasure-table,treasure-profiles}~ GET / PUT | yes | The five ~PerUserStore~ files — opaque per-user JSON round-tripped by the shared GET/PUT pair in =per_user_store.rs=; GET returns ~null~ until the user first persists. |
...
10. *Persistence wrapper* — one ~fooPresetsCmd~ branch in the =Main.update=
    ...
    authenticated sessions PUT the encoded dict to the matching ~PerUserStore~
...
- *Theme* — four variants, all live in the AppBar ⚙ settings popover and in CSS:
  ...
  - *Accessible* (~[data-theme="accessible"]~) — WCAG AAA- target high-contrast
    ...
    slot into the generic per-user store the day they need to roam (one
    ~routers()~ entry — see [[*Backend: a new feature][Backend: a new feature]]).
...
1. *Persistence* — use =JsonFileStore<T>= from =crates/lib/=.  Don't roll a
   ...
   the templates.  If the payload is opaque per-user JSON that only needs an
   authenticated GET/PUT pair, don't write a module at all: adding a sixth store
   to =per_user_store.rs= is one ~routers()~ entry, one ~PerUserStore~ field on
   ~AppState~, and one line in ~RuntimePaths::from_data_dir~.
...
5. *Config* — add fields to the ~Config~ struct in =server/src/config.rs= via
   ...
   + ~Config::resolve_paths~ — every store file is a fixed name inside the data
     dir, and there are deliberately no per-store path overrides.  If the value
     isn't a primitive that ~MergeConfig~ knows about, use the ~extra_cli~ /
     ~extra_file~ / ~skip~ + ~resolve_<field>~ escape hatches — see the OIDC
     pattern in =config.rs= (~ExtraCliFields~ / ~ExtraFileFields~ carry only the
     OIDC trio + ~compendium_claim_user~).
