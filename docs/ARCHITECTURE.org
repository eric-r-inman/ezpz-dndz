...
~cond-checkbox~ and ~cond-radio-plain~ for the choice controls, built by
=View.Inline.Field= so their markup matches too; ~cond-section__caption~ for a
hint, with a warn and a danger variant; and ~cond-fold~ for a section heading
that folds its section, the Save Chain editor's recent-applies log being the
first.  A panel keeps a class of its own only for what it alone has — a treasure
row, a die button, a party list.  Adding an editor means composing these, not
...
~Model.defaultDrawer~, though a stored layout keeps whatever order the GM left.
