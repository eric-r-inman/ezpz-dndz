...
- *Backend* — Rust workspace with three crates: a CLI (~ezpz-dndz-cli~), a
  ...
  (~ezpz-dndz-lib~) holding the ~Creature~ schema, ~JsonFileStore<T>~, the
...
- *Frontend* — Elm 0.19 SPA, served as static files by the Rust server.  The
  server falls back to ~index.html~ for any unknown path so client-side routing
  keeps working.
- *Build* — ~cargo~ for Rust, ~elm make~ for Elm.  Both run inside a Nix dev
  shell defined by ~flake.nix~ (which itself delegates to
  ~foundation.lib.mkRustProject~).  ~just dev~ and ~just serve~ auto-enter the
  dev shell if needed.  The dev shell also carries ~pkgs.poppler-utils~
  (~pdftotext~ / ~pdftoppm~) for SRD-audit workflows that extract the WotC PDF
  to text.
...
- =cli/= — ~ezpz-dndz-cli~ binary.  Body is a ~#[foundation_main]~- wrapped
  ...
  infer-habitats,promote-to-bundle}~, ~encounter {show,count}~, ~dice
  {count,tail,clear}~, ~users {reset-password}~ (operator- only — opens the
  on-disk ~users.json~ directly and writes a fresh Argon2id hash, no server
  required).
- =server/src/=
  - ~main.rs~ — ~#[foundation_main]~ entry point.  Loads the per-user stores,
    hands them to ~Server::with_state~, layers ~require_auth~ over the ~/api/*~
    subtree, and merges the auth + dice + compendium + encounter sub-routers.
  - ~lib.rs~ / ~web_base.rs~ — module index + the slimmed ~AppState~ (just app
    stores; foundation's ~BaseServerState~ rides through
    ~impl_server_state!(AppState, base)~).
  - ~config.rs~ — ~MergeConfig~-derived ~Config~ with two skip-field resolvers
    (~paths~ / ~oidc~) handling cross-field rules MergeConfig can't model
    directly.
  ...
  - ~dice.rs~ — ~/api/dice/*~ (per-user history; ~UserId~ keys the on-disk map).
  - ~compendium/~ — ~/api/compendium/*~ (mod / store / error)
    + ~/api/compendium/saves/*~ (~saves.rs~) + per-user
      ~/api/compendium/groups/*~ (~groups.rs~, ~CompendiumGroupStore~ —
      ~HashMap<UserId, Vec<Group>>~).  The shared 330-creature catalogue is
      global (any logged-in user can read / write); each user's named snapshots
      and creature-groups live under their own ~UserId~ key.
  - ~encounters/~ — ~/api/encounter~ (per-user live encounter in ~store.rs~ —
    ~HashMap<UserId, Value>~) + ~/api/encounter/saves/*~ (~saves.rs~, also keyed
    by user) with conflict / not-found / name-validation semantic errors.
  - ~prefs/~ — Phase 11 stub (mounted in =lib.rs= but not yet wired through
    =web_base= or producing any routes).
- =lib/src/=
  - ~json_file_store.rs~ — ~JsonFileStore<T>~ (mutex + atomic file writes, the
  ...
  - ~users/~ — ~User~ / ~UserId~ / ~UserPublic~ types, ~UserStore~ (Argon2id
    hashing, email-uniqueness check, validation rules).
...
is now the application shell only — every modal, view, update branch, and
per-feature UI substate lives in its own module.
...
=CompendiumTest=, plus =Encounter/{RosterTest,LifecycleTest,XpTest}=.
...
- ~savedSnapshot : Maybe Encounter~ + ~savedAs : Maybe String~ — the most recent
  ...
  / Load success and consumed by Reset (which restores from the snapshot and
  forces ~round = 0~).
- ~pendingControl : Maybe PendingControl~ — two-step confirmation state for the
  destructive Encounter Controls actions (~PendingReset~ / ~PendingClear~).
  When ~Just~, the controls panel renders an inline confirmation banner in place
  of the button grid so a misclick can't drop combat state.
- ~panelCreaturePin : Maybe { id, name }~ — the side-panel pinned creature.
...
- ~xpScope : XpScope~ + ~xpFilterOpen : Bool~ — encounter-bar XP scope filter
  (Enemies & NPCs / Enemies Only / NPCs Only / Selected Only) and the controlled
  open state of its dropdown.  The dropdown was migrated off native
  ~<details>/<summary>~ to a controlled ~<div>~ so global Esc + click-outside
  subscriptions in =Main.subscriptions= can close it.
- ~settingsOpen : Bool~ — AppBar settings popover (theme picker today; density /
  sound when those land).  Same controlled- ~<div>~ + Esc + click-outside
  pattern as the XP filter.
- ~controlMenu : Maybe ControlMenu~ — which Encounter Controls split-button
  dropdown (Save / Load) is currently showing its Server / Device picker.
  ~Maybe~-of-ADT enforces the "only one open at a time" invariant in the type
  system; ~Nothing~ collapses both.  Compendium-modal popovers (Clear / Import /
  Export) follow the same shape but live on the compendium UI substate as
...
Two modals — the dice roller and the compendium browser — sit *outside* the ADT
because their substate has to survive a modal close (dice history, compendium
browser cache + filter).  They expose ~open : Bool~ on their own substate and
live as =Model.dice : DiceUi= / =Model.compendium : CompendiumUi=.
...
~round = 0~ is the pre-combat sentinel: the queue is set up but no one has acted
yet.  Reset / Clear / Load all force the live encounter to round 0.  In this
state the green action button on the controls panel reads "Run Encounter" and
dispatches ~EncounterRun~, which calls ~Encounter.run~ — bumps round to 1 and
sets the active creature to the queue head (highest initiative).  Once round >=
1 the same button reverts to "Next Turn".

Turns advance through =Encounter.Lifecycle.nextTurn=, invoked by
~Update.Encounter.nextTurn~ when the user clicks ⏭ Next Turn.  The lifecycle
module owns:
...
: Bool~ flag.  Initiative-modal entry points ("Disadv. & Surprised" and "Apply &
Sort w/Surprised") flip the flag on whichever creatures are in scope before the
roll / apply runs.  Card row 1 and the encounter title-bar active-creature
summary both render a 😲 badge to the left of the name while surprised.  The
end-of-turn lifecycle hook (~applyEndOfTurn~) clears the flag for the outgoing
creature, so Surprised burns off after the surprised creature finishes their
first turn — matching the rule.  Surprised creatures are suppressed from the
LA-availability reminder strip but not the Special Reactions strip (the rule
only bars LA use, not ordinary reactions).

** Creature record extensions

~Encounter.Creature~ gained six fields beyond the original combat core to
...
- ~surprised : Bool~ — set by the initiative-modal "Disadv. & Surprised" /
  "Apply & Sort w/Surprised" entry points and cleared by ~applyEndOfTurn~ after
  the creature's first turn.  Drives a 😲 badge on the card + title bar and
  suppresses the creature from the LA-availability strip.
...
=Encounter.Wire= encodes all six; the decoder uses ~optional~ with safe defaults
(~False~ / ~""~) so legacy persisted encounters and saved snapshots round-trip
cleanly.
...
~settingsClose~), the Encounter-Controls split-button popovers
(~controlMenuToggle~ / ~controlMenuClose~), and ~NoOp~.  Anything that's pure UI
plumbing at the boundary between the Elm runtime and the workspace shell goes
here rather than inflating =Main.elm=.
...
users can browse the encounter manager, edit creatures, customize cards, etc.,
without ever signing in.  See [[*Anonymous mode][Anonymous mode]] for the localStorage persistence
layer that makes that work.
...
body content and a Stop-hook–style focus redirect in =index.html=, so keyboard /
SR users land on the close button when a modal opens and Tab wraps within the
...
— live encounter, named saves, dice roller, customizable card layouts,
compendium with full CRUD — backed by ~localStorage~ instead of the server.
Signing in auto-migrates the relevant local data to the user's account.  See
[[*Anonymous mode][Anonymous mode]] for the architecture.
...
~created_at~.  Storage is a flat ~Vec<User>~ in =users.json= via
~JsonFileStore~.
...
The bundled SRD set is *not* on disk for the server — it lives in the binary via
~include_str!~ and is merged into every read at request time.  See
[[*Compendium: bundled vs per-user][Compendium: bundled vs per-user]] for the full split and migration story.
...
- *Per-user creatures* (custom creations, edits, duplicates) live in
  ~UserCompendiumStore~ at =<data_dir>/compendium/user-creatures.json= keyed by
  ~UserId~.
...
=<compendium_dir>/.split-v1= is absent:
...
Existing dev-mode JSON of an older flat shape will fail to deserialize and
~JsonFileStore::load_or_default~ falls back to an empty map (logged at WARN).
Fine for dev.
...
card-layouts, or dice-history fetches — each one's destination differs between
auth states (server vs.  localStorage adopt vs. public bundled JSON).
=Update/Auth.elm='s ~meReceived~ owns the per-branch dispatch:

- ~Ok user~ — fires the four authenticated data fetches (~/api/encounter~,
  ~/api/compendium/creatures~, ~/api/compendium/groups~, ~/api/card-layouts~)
  plus the dice- history fetch, plus three optional migration Cmds for any
  ~localStorage~ data the anonymous session left behind (see [[*Anonymous mode][Anonymous mode]]).
- ~Err _~ — adopts every ~localStorage~ snapshot into ~Model~ (encounter, card
  layout, dice history, compendium creatures
  + groups, named encounter saves, named card-layout saves) and falls back to
    ~/bundled-creatures.json~ for the compendium if the local snapshot is
    absent.
...
- *Authenticated* — Encounter / display name link to ~/me~ / About / Donate / ⚙
  settings.  (The *Customize card* button and the *Custom: on/off* toggle are
  commented out for launch — see [[*Customizable creature card][Customizable creature card]].)
...
compendium browser, dice roller, and customizable cards as authenticated users —
the only differences are storage destination and a handful of sign-in nudges on
...
~next : Model~, fires four diff-and-persist Cmds in parallel for the four
most-mutated surfaces:
...
Each helper (~persistEncounterFor~ / ~persistCardLayoutFor~ / etc.) branches on
~next.auth~: ~Authenticated~ → server HTTP, ~Anonymous~ → outbound port to
~localStorage~, ~Loading~ → no-op (brief boot window before the auth probe
lands).  =Update.Dice= follows a slightly different shape because each roll's
existing =Effects.persistDiceRoll= POST is suppressed when anonymous and the
wrapper writes the truncated full list to ~localStorage~ instead; the result is
the same diff-and-write discipline.

A small set of Msgs is excluded from ~shouldPersistAfter~: ~EncounterLoaded~ /
~EncounterPersisted~ / ~AuthMeReceived~ / ~LocalEncounterMigrated~ /
~LocalCardLayoutMigrated~ / ~LocalCompendiumMigrated~ — these flow data IN from
the authoritative source, so re-writing it back would be wasted work or an
infinite loop.

** Boot flag flow

=index.html='s inline boot script reads every ~localStorage~ key synchronously
before ~Elm.Main.init~ and hands them in as flags:
...
~Json.Decode.Value~ Maybe fields stash the raw JSON on ~localCompendiumRaw~ /
~localEncounterRaw~ / ~localCardLayoutRaw~ / ~localDiceHistoryRaw~ for one-shot
adoption — they're cleared after =Update.Auth.meReceived= picks a branch, so the
model doesn't keep the (possibly-large) JSON value around after boot.
...
~clearLocalEncounter~ / ~clearLocalCardLayout~ / ~clearLocalCompendium~ ports
and push a success toast naming the slot.  Failures toast but leave the local
...
auth state); click → ~/login~; tooltip swaps to a sign-in nudge.  The
PanelControls Save/Load "To Server" / "From Server" menu items use a different
shape — labels rename to "To Browser" / "From Browser" anonymously (via
~saveToServerLabel auth~) and the click dispatches the real Save modal, which
then routes through =Update.Save= / =Update.Load='s auth-branched mutators.

* Persistence

The server-side persisted JSON files all live under ~--data-dir~ (default ~./~).
Each is read at boot and atomically rewritten on every mutation through
~JsonFileStore<T>~ from =crates/lib/=.  For the parallel ~localStorage~ shapes
used by anonymous sessions, see [[*Anonymous mode][Anonymous mode]] — every server-backed surface
below has a matching ~localStorage~ key that the ~Main.update~-wrapper
diff-and-write pipeline maintains.
...
The ~JsonFileStore<T>~ pattern in =crates/lib/= is the canonical persistence
helper.  Every server-side feature with a JSON backing file should use it rather
...
The middle pane's button grid runs a small set of named flows that the rest of
the app routes through:

- *Save / Load* are split-button dropdowns ("💾 Save ▾" / "📁 Load ▾"); the
  trigger toggles a popover whose menu items pick destination (Server / Device).
  ~Maybe ControlMenu~ on ~Model~ tracks which popover is open; subscriptions in
  =Main.subscriptions= close on Esc / click-outside.  Picks dispatch into the
  destination-aware modal:

  - *Save → Server / Device* — opens =View.Modal.Save= pre-set to the chosen
    radio.  Filename input, list of existing server saves with inline rename /
    overwrite / delete.  Server uploads PUT to ~/api/encounter/saves/:name~
    (server returns 409 on a name collision, surfaced as an inline
    overwrite-confirm banner; ~?overwrite=true~ upserts on confirm).  Device
    downloads go through ~File.Download.string~.
  - *Load → Server / Device* — Server opens =View.Modal.Load= (list of saves,
    click-to-load with confirm); Device fires ~File.Select.file~ directly
    without the modal.  Either path replaces the live encounter and forces
    ~round = 0~ + ~activeName = ""~ so the GM lands in pre-combat mode.
    Successful loads also populate ~savedSnapshot~ + ~savedAs~.

- *Reset* / *Clear* — staged through ~pendingControl~ so a misclick can't drop
  combat state.  First click swaps the button grid for an inline confirmation
  banner (Cancel / coloured Confirm); second click applies.
  - *Reset* — keeps every creature in the roster but strips per-fight state: HP
    ...
    note, memo, compendium id) preserved.  Round → 0, active creature → none, so
    the GM lands back in the pre-combat "Run Encounter" state.
...
The right-hand panel pins one compendium creature's stat block, chosen by
clicking a creature card's name in the encounter pane.  Lookup is
*id-then-name*: =View.PanelDetail.resolvePin= first calls ~Compendium.find
pin.id~ and on miss falls back to ~Compendium.findByName~ on the encounter
creature's display name (stripped of the numeric instance suffix via
...
differently in two contexts.  In the compendium modal and the standalone view,
...
same pattern as the Encounter Controls Save / Load triggers.  Mediating state
lives on the compendium UI substate as ~bulkMenu : Maybe CompendiumBulkMenu~
(~ClearMenu | ImportMenu | ExportMenu~), so only one popover is open at a time
and the global Esc / click-outside subscriptions have a single handler to close
them all.
...
The Save / Load Compendium modals themselves mirror the encounter Save / Load
modals: filename input, list of existing snapshots, inline overwrite-confirm
banner.  The MVP omits per-row rename / delete affordances — re-saving under an
existing name fires the overwrite-confirm prompt, which covers the common case.
...
- *Compendium list rendering* — groups render at the top of the list as
  ...
  =View.Modal.Compendium.groupListItem= produces a header + an optional list of
...
- *Add to encounter* — clicking a group row pins it in the right pane with an
  action bar (➕ Add Group to Encounter / ✏️ Edit / 🗑 Delete).  Add dispatches
...
*Deferred for launch.* Every supporting module below still compiles and tests
pass, but the AppBar entry points (the *🎨 Customize card* button and the
*Custom: on/off* toggle) are commented out and =View.Workspace.renderCard=
unconditionally routes through =View.Card.view= regardless of
~model.useCustomCardLayout~.  The plumbing — Ui / Update / View modules,
~/api/card-layouts~ routes, ~localStorage~ snapshot, sign-in migration — is
retained so the feature can be re-enabled behind one AppBar diff once the widget
set is finished.  ~Update.Auth.applyLocalCardLayout~ still calls
~Card.Layout.migrateLegacyDefault~ on anonymous boot, but the result is
currently dead in user-visible terms.

The original design notes follow.

Users can rearrange the creature card's contents to taste — pick which widgets
render, in which rows, with what alignment, and switch the encounter queue
between vertical-list and tiled-grid arrangements.  Modeled on Wordpress-style
widget composition.

** Data model

[[file:../frontend/src/Card/Layout.elm][frontend/src/Card/Layout.elm]] owns the layout types: a ~CardLayout~ is a list of
~CardRow~ s; each row is a list of ~CardWidget~ s + a ~RowAlignment~ (left /
center / right / space-between).  The ~CardWidget~ enumeration is the *stable
catalogue* of everything today's card is capable of rendering — add a variant
here (and to ~widgetAllValues~) when a new widget lands on =View.Card=.
~QueueView~ (~ListView | GridView~) is the queue-level layout choice.

A ~defaultLayout~ approximates today's hardcoded card.  Users who haven't
customized see this; the editor opens to it the first time.
...
Reset-to-defaults is in the editor itself — clicking the *↺ Reset to defaults*
button copies ~Card.Layout.defaultLayout~ back over the in-progress layout (it
does *not* touch any saved record on the server).

** Encounter wiring

[[file:../frontend/src/View/Workspace.elm][frontend/src/View/Workspace.elm]]'s ~panelMain~ picks between =View.Card.view=
(classic, full inline edit) and =View.Card.Custom.view= (layout-driven) based on
~model.useCustomCardLayout~, toggled by the *🎴 Custom: on/off* AppBar button.
The ~queueView~ field drives the ~.creature-grid--list~ vs
~.creature-grid--grid~ class on the panel body.

The custom renderer wires Msg-firing handlers for the action buttons (Damage /
Heal / Temp HP / Condition), status toggles (concentrating / hiding / dodging /
flying / holding / skip), row controls (×, ⧉, panel pin), memo + timer slots,
and the selection checkbox.  Inline click-to-edit on HP / AC is read-only in
this prototype — GMs adjust HP via the Damage / Heal / Temp HP modals; AC via
...
the shared ~Effects.pushDiceRoll~ + ~persistDiceRoll~ pipeline so the dice modal
...
history *without* opening the dice modal.  The panel's Roll button picks up its
...
Each history row in the dice modal carries a ↻ rerun button.  The button is now
...
The right cluster of the encounter title bar runs a real XP total, not a
placeholder.  =View.EncounterBar.totalXp= walks the encounter, resolves each
creature through ~Compendium.find creatureId~, and sums two parallel running
totals: ~total~ (every in-scope creature's base ~xp~) and ~lairTotal~
(~xpInLair~ when present, falling back to ~xp~).  Players are always excluded;
missing-from-compendium entries are silently skipped.  When ~lairTotal > total~
the view renders a muted secondary chip "(N w/Lair)" alongside the base figure.

The four scope options drive the dropdown filter via the ~Encounter.Xp.XpScope~
enum (~Enemies & NPCs~ / ~Enemies Only~ / ~NPCs Only~ / ~Selected Only~).
~Selected Only~ uses the row 1 multi-select checkboxes; the other three resolve
via the compendium creature's ~kind~ tag.
...
Compendium.Db -> Totals~ + ~formatThousands~.  =View.EncounterBar= unwraps the
...
The dropdown itself was migrated from native ~<details>/<summary>~ to a
controlled ~<div>~ because the native element couldn't be closed by Esc or by
clicking outside.  ~Model.xpFilterOpen~ holds the open state; new
~XpFilterToggle~ / ~XpFilterClose~ Msgs flip / clear it; the existing
~XpScopeSet~ also closes on selection.  When open, =Main.subscriptions= layers
two extra subs in via ~Sub.batch~: ~Browser.Events.onKeyDown (escKey
XpFilterClose)~ and ~Browser.Events.onMouseDown (Decode.succeed XpFilterClose)~.
An internal ~stopPropagationOn "mousedown"~ on the dropdown wrapper keeps the
toggle click from auto-closing.
...
next to the creature name in every ~TagDisplay~ mode (compendium modal, pinned
right rail, standalone view).  CSS variants in =style.css= colour them blue
(Player), red (Enemy), and yellow (NPC) so the GM can pick a creature out of the
queue at a glance.
...
- Replace mode — ~Ui.QuickAdd.freshForReplace oldName~ flips the modal title to
...
- *Quick Add modal* — the pinned italic ~Placeholder~ entry at the top of the
...
The Condition modal and Timer modal each carry a Save / Load footer that
remembers full configurations under user-chosen names.  Both modals follow the
same templatable shape so new modals can adopt the pattern without re-inventing
it.
...
5. *Port* — ~Ports.persistLocalFooPresets : E.Value -> Cmd msg~ with a matching
   JS subscriber in =index.html= that writes to ~localStorage.fooPresets~.
...
10. *Persistence wrapper* — one ~fooPresetsCmd~ branch in the =Main.update=
    diff-and-write wrapper, encoded through ~Ui.Foo.Wire.encodePresets~.  No
    auth split — presets live in ~localStorage~ for every session today (no
    server endpoint yet); the codec is reusable as the wire body when that
    lands.
...
The 🎲 Random Encounter modal (opened from the Compendium side panel) builds an
...
=Ui/RandomEncounter.elm=; the modal view in =View/Modal/RandomEncounter.elm=.
Bundled lore content lives in =Encounter/RandomEncounter/Lore.elm=;
user-authored lore lives on =model.userLoreGroups= and persists via
...
the rule-flavored side effects.  The inline click-to-edit on row 1 (AC) and row
2 (current / max HP) of each card all flow through these via the shared ~HpEdit~
/ ~HpField~ / ~HpEditStart~ machinery — same Msg constructors, different field
discriminator.
...
- *Theme* — four variants, all live in the AppBar ⚙ settings popover and in CSS:
  ...
  - *Accessible* (~[data-theme="accessible"]~) — WCAG AAA- target high-contrast
    palette, bumped font sizes across every panel + stat block + dice modal + CR
    ...
    Same code path runs anonymous + authenticated; per-account preferences will
    move to ~/api/me/preferences~ once that endpoint lands
    (=crates/server/src/prefs/= is a stub today).
...
When ~/api/me/preferences~ lands, ~Ports.savePreferences~ becomes a fallback for
...
by a tiny vanilla-JS script in [[file:../frontend/public/index.html][frontend/public/index.html]].  The script delegates
...
past 1k — it sits around 1.4k today, almost entirely dispatch-table boilerplate
(one ~MyMsg arg -> Update.Foo.bar arg model~ line per Msg branch plus the
modal-view + subscriptions composition).  The whole point of that work was to
make new-feature additions follow a templatable pattern — every modal, every
domain feature, every CLI subcommand has a canonical home now.  Don't grow
...
Most new features in the GM-side UI are modal-shaped.  The canonical layout, in
dependency order:
...
2. *UI substate* — =Ui/Foo.elm= defines the modal's record alias and a ~fresh :
   ... -> FooUi~ constructor.  Only fields the modal cares about; transient
   typing state is fine here.
3. *Modal ADT* — add ~ModalFoo FooUi~ to the ~Modal~ ADT in =Model.elm=.  *Do
   not* add a separate ~Maybe FooUi~ field on ~Model~.  The ADT is what
   type-enforces "only one modal open at a time."
...
5. *Update branches* — =Update/Foo.elm= exposes one function per Msg branch.
   ...
   ~model.modal~; the ~withFooUi~ lens pattern-matches on ~Just (ModalFoo ui)~
   and is private to the module.
6. *Cmd-emitting helpers* — go in =Effects.elm= (or =Util/Http.elm= for HTTP
   boilerplate).  Update modules don't ~import Http~ directly.
7. *View* — =View/Modal/Foo.elm=.  Entry point is ~view : Model -> Html Msg~
   that pattern-matches on ~model.modal~ for ~Just (ModalFoo ui)~ and falls
   through to ~text ""~.  Use =View.Modal.view= for the chrome (backdrop,
   header, ✕ close, click-out, Esc-close).
8. *Tooltips* — every ~title~ attribute the view sets goes through a constant in
   =View/Tooltips.elm=.  No inline ~title "..."~ literals.
9. *Dispatch wiring* — add one line each to =Main.view= (call the new view),
   =Main.subscriptions= (the modal's Esc-key case), and =Main.updateInner= (each
   Msg branch is a one-liner: ~FooMsg arg -> Update.Foo.bar arg model~).
10. *Tests* — =frontend/tests/Foo/BarTest.elm= for the domain helpers; the
...
1. *Persistence* — use =JsonFileStore<T>= from =crates/lib/=.  Don't roll a
   one-off file store.  The dice store, encounter store, and compendium store
   all use it; new features should match.  If the data is per-user, the
   canonical shape is ~JsonFileStore<HashMap<UserId, T>>~ with operations taking
   a leading ~&UserId~ parameter — see =saves.rs= / =encounters/store.rs= for
   the templates.
...
5. *Config* — add fields to the ~Config~ struct in =server/src/config.rs= via
   ...
   Cross-field defaults that depend on ~--data-dir~ flow through ~RuntimePaths~
   + ~Config::resolve_paths~.  If the value isn't a primitive that ~MergeConfig~
   knows about, use the ~extra_cli~ / ~extra_file~ / ~skip~ + ~resolve_<field>~
   escape hatches — see the OIDC + path-override pattern in =config.rs=.
...
Subcommands operate file-direct (read and write JSON synchronously) so the CLI
keeps working when the server isn't running.  The =compendium= / =encounter= /
=dice= modules are the templates.
...
- *Nix* — flake input.  =flake.nix= owns the per-system assembly (rust
  ...
  by convention; ezpz-dndz uses this for the server crate's bundled-Elm-frontend
  build.  =flake.nix= also splices in ~foundation.lib.cargoHuskyHookSnippet
  pkgs~ inside its ~shellHook~ so the ~.cargo-husky/hooks/~ pre-commit hook is
  symlinked into ~.git/hooks/~ on devShell entry.  ~mkNixosService~ /
  ~mkDarwinService~ collapse the two service-module wrappers under
  =nix/modules/= to ~30 lines each.
...
- ~Server~ runner (~Server::new~, ~with_state~, ~merge~, ~listen~),
  ~BaseServerState~ (registry / counter / OIDC client / frontend path), the
  ~impl_server_state!~ macro that splices the base into a custom ~AppState~.
- Health / metrics / OpenAPI / Scalar / SPA-fallback / session- layer routes and
  middleware.
...
- Reusable GitHub Actions workflows
  (=reusable-{ci,publish,branch-protection,dependabot-automerge}.yml=) the
  project's CI files call into.
...
is the base for the next diff-based compliance pass.  Three entries today:

1. =51239f87…= — initial template adoption.
2. =992414005ea1ce9…= — env-var lowercase / POSIX §8.1 pass.
3. =6d65d3d8…= — current sync (combinator-preference lints, pre-commit ACMR
   filter, ~mkRustPackages~ helper rewrite).
