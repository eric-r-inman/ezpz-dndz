...
~cond-section__caption~ for a hint, with a warn and a danger variant; and the
~log-head~ / ~log-fold~ / ~log-list~ set for a log behind a fold.  A panel keeps
a class of its own only for what it alone has — a treasure row, a die button, a
