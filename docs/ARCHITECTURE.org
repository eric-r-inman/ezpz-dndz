...
Saving Throws property line — all fire the same triple-roll:
~Update.Dice.tripleRollCmd~ rolls ~1d20 + bonus~ three ways at once (standard,
...
strip next to the 🎲 icon — the four most recent totals — shows the same three,
...
the shape below leaves out: a name already in the list stages ~SurfaceConfirm~,
so a save cannot quietly replace a preset.  Both follow the same templatable
...
8. *View* — =View/Modal/Foo.elm= renders two controls, Save in the footer and
   ...
   - The Save control — a single ~Save~ button when ~pendingSaveName ==
     Nothing~; otherwise the naming rows, one each for the name, its category,
     and the commit with Cancel beside it.  The commit reads as an overwrite
     when the name is already in the list, the GM's own or a bundled one.
