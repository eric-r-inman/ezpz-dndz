...
| Region          | Modules                                                                                                                                 |
|-----------------+-----------------------------------------------------------------------------------------------------------------------------------------|
| Application     | =Main.elm= (boot + dispatch table), =Model.elm= (record + Modal ADT), =Msg.elm=, =Hotkeys.elm= (the keyboard shortcuts)                  |
| Auth            | =Auth.elm= (~AuthState~ ADT + ~User~ + decoder)                                                                                          |
| Domain          | =Encounter.elm= + =Encounter/{DeathSaves,Difficulty,Lifecycle,RandomEncounter,Roster,SaveChain,SaveNotice,Seed,Treasure,Wire,Xp}= (plus the =RandomEncounter/Lore=, =SaveChain/{Bundled,Wire}=, and =Treasure/*= sub-modules) |
| Domain          | =Compendium.elm= + =Compendium/{Group,GroupWire,Parser,Reference,SpellcastingText,Wire}= |
| Domain          | =Dice.elm=, =DrawerLayout.elm=, =HpChange.elm=, =Preferences.elm=                                                                                            |
| Routing / Cmd   | =Route.elm= (~Home / Login / Me / Donate / About / CompendiumCreaturePage / Compendium / NotFound~), =Effects.elm= (every Cmd-emitting helper, HTTP included, plus the ~shouldPersistAfter~ / ~shouldBroadcastAfter~ persistence predicates), =Ports.elm= (outbound JS — ~savePreferences~, the ~persistLocal*~ ports, the three ~clearLocal*~ sign-in-migration ports, and the cross-tab BroadcastChannel ports) |
| UI substate     | =Ui/{Account,Compendium,Condition,CrCalculator,Dice,Duplicate,GroupEdit,HpChange,Initiative,LoadCompendium,Login,LoreEdit,Memo,ModalChrome,Note,PlaceholderRename,QuickAdd,RandomEncounter,SaveChain,SaveCompendium,SaveLoad,Timer,Toast,Treasure,TreasureTable}.elm= |
| Update / shell  | =Update/Shell.elm= (URL routing, ~/me~, encounter persist response, settings popover, ~NoOp~), =Update/Tabs.elm= (the cross-tab BroadcastChannel receive path, ~encounterFromOtherTab~) |
...
| View / surfaces | =View/Modal/*.elm= for what still opens over the page and =View/Panel/*.elm= for what opens in the editor column's drawer; =View/Modal.elm= and =View/Panel.elm= carry the two shells, whose fields line up but for the optional panel ~close~ |
| View / pages    | =View/Page/{Loading,Donate,NotFound,Compendium,CompendiumStandalone}.elm= — the page trees ~Main.viewPage~ dispatches to (formerly inline in =Main.elm=) |
...
- ~savedSnapshot : Maybe Encounter~ + ~savedAs : Maybe String~ — the encounter
  as it stood at the most recent save or load, and the name it went under.  Set
  by Save / Load success; the snapshot backs the unsaved-changes dot in the app
  bar and the name pre-fills the Save modal.
...
- ~xpScope : XpScope~ — which creatures the XP total counts.  The scope is
  picked in the XP panel, which is also where the readout lives; the picker's
  open state is the ~SurfaceXp~ marker in the drawer stack, not a field.
...
with nobody active.  In that state the editor column's Turn button dispatches
...
2. *Condition lifecycle hooks* — bracketing the queue walk: ~applyEndOfTurn
   ...
   ticks every ~DurationCountdown AtEnd~ that counts the outgoing creature's
   turns, whoever carries it, and expires every ~DurationUntilTurn AtEnd <name>~
   across the encounter; the begin-of-turn pass does the same for the ~AtBegin~
   phase.

** Creature record extensions

~Encounter.Creature~ gained several fields beyond the original combat core to
support placeholder stubs and stat-block-style metadata:
...
- ~hasSpecialReactions : Bool~ — copied from the compendium template at spawn
  time.  When set, the creature shows up in the "Special reactions: …" sticky
  strip beneath the title bar.  Toggleable per-creature via a checkbox in the
  compendium editor; auto-set on 22 bundled SRD creatures (Hydra, Marilith,
  Vampire, mephits, …).
...
<Condition>" notice on success — the same notice a manual chip-roll success
posts.
...
path: ~encounterFromOtherTab~ adopts an encounter another tab broadcast.  The
outbound side stays in the =Main.update= wrapper, guarded by
~Effects.shouldBroadcastAfter~ so a received broadcast is never re-broadcast.
...
CompendiumStandalone) and =Main.elm= only picks which one to call.
...
CASCADE~ back to ~users~), defined across the migrations under
...
| ~condition_preset_sets~ → ~condition_presets~ → ~condition_preset_companions~ | 0002, 0005, 0006, 0007 | Saved Add-Condition forms.                                        |
| ~save_chain_sets~ → ~save_chains~ → ~save_chain_effects~ | 0002, 0005, 0006 | Save Chain preset recipes.                                        |
...
| ~encounters~ (+ creatures / conditions / save notices / recharge abilities / pips / treasure + its contributions / items) | 0004, 0005, 0006, 0008 | The live encounter, fully typed — treasure settings flatten onto the ~encounters~ row itself; absent row reads as ~null~. |
...
- ~Err _~ — adopts every ~localStorage~ snapshot into ~Model~ (encounter, dice
  history, compendium creatures + groups, presets, party, lore groups, treasure
  table) and falls back to ~/bundled-creatures.json~ for the compendium if the
  local snapshot is absent.
...
- *Anonymous (any route except ~/login~)* — same buttons, but the display-name
  ...
  the brand: *"Sign in to keep changes."*
...
Save/Load Compendium modals, and named encounter saves via the Encounter menu's
...
| Editor-column layout | (localStorage in both modes)       | =drawerLayout=                  |
...
drawerLayoutCmd        = if Model.drawerLayout next /= Model.drawerLayout model ...
#+end_src

The layout branch is the one that diffs a derived value rather than a ~Model~
field: the panels carry more than the arrangement, so comparing them whole would
fire on every fold and re-aim.
...
| =localDrawerLayout=      | =drawerLayout=         | ~Model.drawer~ via ~Model.applyDrawerLayout~ |
...
auth state); click → ~/login~; tooltip swaps to a sign-in nudge.  The Save and
Load modals use a different shape — the server destination shows an anonymous GM
a sign-in notice where the account's saves would be.
...
| ~/api/encounter/saves(/:name)*~   | ~encounter_saves~ + ~encounter_save_nodes~            | yes       | User-named encounter saves (the Encounter menu's Save and Load modals).  Opaque bodies, exact echo.  PUT supports ?overwrite=true; rename / delete endpoints sit alongside; semantic 409 / 404 / 400 errors. |
...
- *Legendary Action available* — ~legendaryActionStrip~.  Lists every queue
  member with legendary actions left, comma-separated, with the remaining count
  in parentheses.  Each name is a button that fires ~QueueScrollTo~ to bring the
  creature's card to the top of the queue.  Filters: not the currently-active
  creature (rule: LA fires on OTHER creatures' turn-ends), not down, not dead.
- *Special reactions* — ~specialReactionsStrip~.  Lists every queue member whose
  ...
  name-button affordance.  Filters: not down, not dead.  Does NOT filter the
  active creature — ordinary reactions can fire on the bearer's own turn.
...
The app runs a small set of named encounter flows that the rest of it routes
through:

- *Saves* — two modals off the Encounter menu, =View.Modal.SaveLoad=, one each
  way.  The row affordances sit on the load side.

  - Server uploads PUT to ~/api/encounter/saves/:name~ (the server returns 409
    on a name collision, surfaced as the modal's overwrite-confirm banner;
...
* Stat block under a card

Clicking a creature card's name unfolds the compendium creature's stat block
directly beneath that card in the encounter pane, attached to it and scrolling
with the queue.  A bar across its top names the creature and holds the Minimize
button, and stays at the top of the scroller while a long block scrolls under
it.  Several cards can have theirs unfolded at once: the set lives in
~model.openStatBlocks~, keyed by creature name, and ~Model.reaimStale~ prunes a
name whose creature has left the queue.  The spell list, which names a creature
from elsewhere, fires ~StatBlockShow~, which also scrolls the card into view;
the card's own name fires ~StatBlockToggle~ and the bar's title
~StatBlockMinimize~.

Lookup is *id-then-name*: =View.Card.StatBlock= first calls ~Compendium.find~ on
the card's compendium id and on a miss falls back to ~Compendium.findByName~ on
the encounter creature's display name (stripped of the numeric instance suffix
via ~Encounter.Roster.instanceBaseName~).  The fallback exists because old saved
encounters can carry ~creatureId~ values that no longer match the bundled
compendium UUIDs (e.g. a creature was originally pasted under a provisional id).
When neither lookup matches, the block shows an explicit empty-state message
...
TagBadgesOpenInNewTab~) because only the compendium page wants the ↗ link on the
name row; every mount renders user tags as a right-justified strip of small
badges (=.statblock__tag-badge=), and nothing when ~tags~ is empty.
...
The Save / Load Compendium modals themselves mirror the Encounter menu's Save
and Load modals: filename input, list of existing snapshots, inline
overwrite-confirm banner.  The MVP omits per-row rename / delete affordances —
re-saving under an existing name fires the overwrite-confirm prompt, which
covers the common case.  Successful save / load clears ~compendiumDirty~ and
stamps the snapshot's name into ~CompendiumUi.savedAs~ so the next Save modal
opens pre-populated.
...
- *Add to encounter* — clicking a group row pins it in the editor column's panel
  with an action bar (➕ Add Group to Encounter / ✏️ Edit / 🗑 Delete).  Add
...
* Triple-rolls: attack / ability check / saving throw

Three click targets on a stat block — an inline "+N to hit" attack link, one of
the six ability cells (STR / DEX / CON / INT / WIS / CHA), and a chip in the
Saving Throws property line — fire the same triple-roll, and so does the editor
column's "20" button, which stands for no creature and adds no bonus:
~Update.Dice.tripleRollWith~ rolls ~1d20 + bonus~ three ways at once (standard,
advantage, disadvantage) as a single ~Dice.batchRollCmd~ so the three don't
share a same-millisecond RNG seed, landing together in ~TripleRollLanded~.  The
bonus is the attack modifier, the flat ability modifier, or the proficient save
bonus (~savingThrows~ entry when the creature has one) depending on which of the
three fired it; the ability/saving-throw paths route through
=Update.AbilitySave.trigger=, which only adds the dice-history label
(~"<ABILITY> check"~ vs ~"<ABILITY> saving throw"~) before delegating — the
attack path tags its own rolls ~"Attack"~ directly.

The landing handler (~Update.Dice.tripleRollLanded~) pushes all three rolls
through the shared ~Effects.pushDiceRoll~ + ~persistDiceRoll~ pipeline like any
other roll, spawns three colour-coded floating popups spread out side-by-side at
the click position (white / green / red for standard / advantage / disadvantage
— ~Model.PopupColor~), and sets ~Ui.Dice.rollBadgeOverride~ so the rail's badge
strip next to the 🎲 icon — the three most recent totals — shows the same three,
its newest three entries, in the same colours even after the popups fade.  Any
single roll landing afterward — from any source — clears the override via
~Effects.pushIncomingDiceRoll~, so the strip reverts to its ordinary recency
view, the newest total emphasized and advantage or disadvantage rolls keeping
their colour, until the next triple-roll.  No HP mutation; the GM applies
effects manually based on the result.

* Inline dice in stat blocks

Every clickable dice-formula link in a stat block (damage rolls, generic inline
notation — everything except the attack link covered above) routes through
~Update.Dice.rollFromStatBlock~, which fires the roll into the shared dice
history *without* opening the dice panel.  The Dice Roller marks its title as
unread so the GM can open the log when they want to see the breakdown.
...
The XP panel's headline reports the encounter's XP total.
~Encounter.Xp.totalsFor~ walks the encounter, resolving each creature through
~Compendium.find creatureId~, and sums two parallel running totals: ~total~
(every in-scope creature's base ~xp~) and ~lairTotal~ (~xpInLair~ when present,
falling back to ~xp~).  Players are always excluded; missing-from-compendium
...
The scope is picked in the XP panel (=View/Panel/Xp.elm=), which carries the
total alongside it so the effect of a pick is visible while making it.  The
~SurfaceXp~ marker in the drawer stack is the panel's open state;
~Model.defaultDrawer~ supplies it at boot, and ~XpScopeSet~ deliberately leaves
it unfolded.  Esc folds it through the drawer's shared path; a click-outside sub
would fire on every click landing in the queue beside the panel.
...
next to the creature name in every ~TagDisplay~ mode.  CSS variants in
=style.css= colour them blue (Player), red (Enemy), and yellow (NPC) so the GM
can pick a creature out of the queue at a glance.
...
A save-to-end may carry a failed-save outcome — damage the bearer takes, a
condition the chip turns into, or both — which ~Update.Condition.saveLanded~
resolves after the roll: the rename clears the save, since a second failure that
petrifies leaves nothing to save against, and a formula's damage rolls through
its own landing message.  The Save Chain editor writes the same record onto the
conditions it applies, and draws the Condition editor's duration picker
(=View.Inline.DurationPicker=) beneath every effect and beside its success-side
immunity.  A preset can't name the creature an "until turn" watches, so its
~EffectDuration~ names a role — the target, the active creature — that
~Encounter.SaveChain.resolveDuration~ settles against the encounter at apply
time, by the Condition editor's own rules for "next turn" and the countdown's
first tick.

Two save timings wait on the GM rather than a turn hook.  "Ask at end of turn"
and the manual mode both pulse the chip — ~Model.flashConditions~ names the
bearer-and-id pairs pulsing, which ~Update.Encounter.nextTurn~ fills from
~Encounter.remindersAt~ and a sleep clears — so the GM rolls from the chip when
the rule's trigger held.  A save that reacts to damage is caught by
~Update.Condition.damageTriggered~, a pass ~Main.update~ runs on every message's
result: a creature whose current-plus-temporary hit points fell took a hit, and
each of its conditions with a damage trigger either pulses or rolls at once.
The pass skips the save-landing messages themselves, or a condition whose failed
save deals damage would roll without end, and the messages that swap in a whole
encounter from storage, whose numbers are not hits.

An area chain (~SaveChain.area~) marks each target with a condition carrying an
~AreaTracker~ — the chain's name plus the ability, DC, and bonus fixed at apply
time, as a save-to-end's are — and ~nextTurn~ rolls those markers at their phase
through ~Effects.areaRollCmdsFor~.  The landing looks the chain up by name in
the presets and resolves the earned side with the auto-rolled amounts
~savesRolled~ uses, so a spell preset's blank DC is filled from the marker.  A
companion effect lands as a second condition whose ~linkedTo~ names the
primary's id; ~Encounter.removeCondition~ and the lifecycle ticks prune
companions whose primary is gone, whichever way it went.

* Editor vocabulary

Every editor in the drawer is drawn from one set of classes, the Condition
editor's: a ~cond-section~ per group of settings, separated by a dashed rule; a
~cond-row~ for each line, which wraps whole items to the next line rather than
clipping or breaking a label mid-text; ~cond-label~ for the words beside a
field; ~cond-input~ and ~cond-select~ for the fields, at one size, with width
modifiers for the two-character, short, name-length, and take-the-room cases;
~cond-checkbox~, ~cond-radio-plain~, and the ~cond-spin~ pair for the choice and
nudge controls, built by =View.Inline.Field= so their markup matches too;
~cond-section__caption~ for a hint, with a warn and a danger variant; and
=View.Inline.Field='s fold heading — the ~fold-head~ row with its ~fold-toggle~
— for a section that folds, with ~log-list~ under it when the section is a log.
An editor's body is an ~editor-body~, which adds no chrome of its own: the panel
body pads it, and each section carries the air on either side of the dashed rule
between sections, so the first row, every rule, and the last row sit the same
distance from what is around them in every editor.  The encounter-level panels
keep their own section classes but follow the same rhythm.  A panel keeps a
class of its own only for what it alone has — a treasure row, a die button, a
party list.  Adding an editor means composing these, not minting a parallel set.

The HP change log lives on the model once and is read two ways: the Manage HP
editor shows every entry, and the dice roller interleaves only the entries a
roll produced with its rolls, on the ~rolled~ flag each entry carries.  Undo
always reverts the newest entry, whichever mount offers it.

The heading row of every panel folds it on a click and, with shift held on an
open panel, folds every panel: ~View.Panel.headerClick~ reads the modifier and
picks between the panel's own toggle and ~DrawerFoldAll~, and the caret button
shares the decoder.  =Hotkeys.elm= binds the column's keyboard shortcuts,
fold-all among them; its table also writes the About page's list of them, so a
new shortcut documents itself.  The tag a panel wears while its feature is still
beta is ~View.Panel.betaTag~ on its ~titleTrail~; those panels also sit last in
~Model.defaultDrawer~.  A stored layout carries the version of the arrangement
rules it was saved under (=DrawerLayout=), and ~Model.applyDrawerLayout~ moves
the beta panels to the bottom of a layout saved before version 2 — once, since
the GM's next change writes the layout back at the current version — leaving a
pinned one where the GM put it.

* Per-surface preset pattern

The Condition and Timer editors each carry Save / Load controls that remember
full configurations under user-chosen names.  The Condition editor adds one step
the shape below leaves out: naming happens in a modal
(=View/Modal/ConditionPreset.elm=), whose commit says whether it will replace a
preset of that name.  Both follow the same templatable shape so a new surface
can adopt the pattern without re-inventing it.
...
6. *Msg family* — eight constructors, the core of the Condition family:
...
8. *View* — =View/Modal/Foo.elm= renders two controls, Save in the footer and
   Load where the editor wants it:
   - The Save control — one button that sets ~pendingSaveName~, which is what
     the naming form reads to know it is wanted.  The commit reads as an
     overwrite when the name is already in the list, the GM's own or a bundled
     one.
...
The 🎲 Random Encounter panel builds an encounter to a party's XP budget and
drops it into the queue.  The pure domain lives in
=frontend/src/Encounter/RandomEncounter.elm=; the UI substate in
...
Every bundled group carries a one-line authored description, shown in the
Compendium's group detail pane.
...
- ~Damage Int~ — temp HP soaks first; current HP clamps at 0.  The death-save
...
The ~Readied~ status toggle on row 2 of every creature card also animates
...
7. *View* — =View/Panel/Foo.elm= for a workspace surface, which is nearly all of
   ...
   comes from =View.Panel.view= or =View.Modal.view=, whose fields line up but
   for ~close~, which only a modal takes.  The ~Header~ is passed straight
   through to =View.Panel.view=, which owns the heading row and decides whether
   the body renders at all.  That row folds the panel when clicked and is the
   drag handle for reordering, so a control placed in ~titleTrail~ takes its
   handler from ~View.Panel.onClickWithoutFolding~ rather than ~onClick~ — a
   plain ~onClick~ there folds the panel as a side effect of doing its own job.
8. *Drawer registration* — a panel also needs a ~SurfaceLens~ in =Model.elm=, a
   render arm in ~View.PanelDrawer.panelFor~, an arm in ~Model.surfaceKey~, and
   — when the drawer boots with it — an entry in ~Model.defaultDrawer~; those
   four move together.  The compiler forces the ~surfaceKey~ arm but not the
   care it needs: those strings are the persisted column layout, so a key is
   chosen once and never renamed.  ~Main.drawerEscSub~ needs an arm only for a
   panel Esc should treat specially; the rest fold.  Panels live in the
   ~model.drawer~ stack — many open at once, newest at the bottom — and their
   Update modules open, fold, and edit them through ~Model.openDrawer~ /
   ~foldDrawer~ / ~mapDrawer~; nothing takes a panel out of the stack, since
   none has a trigger to put it back.  The stack is partitioned: panels the GM
   has pinned hold the top in their own order, and ~Model.drawerDropIndex~
   clamps every move to the side of that boundary it started on, so no reorder
   can interleave the two groups.  Each group scrolls in a region of its own, so
   the pinned panels stay put while the rest scroll beneath them, and an opener
   brings its panel into view through ~Effects.scrollDrawerTo~, which finds the
   region the panel sits in.  A per-creature editor takes a narrower shape: its
   body lives in =View/Inline/Foo.elm= and =View.PanelDrawer='s ~editor~ helper
   supplies the chrome, so every editor's target strip and heading row stay
   identical.  It also needs an arm in ~Model.reaimStale~ that actually re-aims
   — the compiler accepts a fall-through, and a fall-through silently strands
   the editor on a creature that has left the queue.
...
10. *Dispatch wiring* — one line per Msg branch in =Main.updateInner= (~FooMsg
    arg -> Update.Foo.bar arg model~).  Esc needs no wiring unless the panel
    should treat it specially; see step 8.  =View.PanelDrawer= renders a drawer
    panel, so it needs no mounting in =Main.appShell=; a modal does.
