...
state the Actions column's green Turn button dispatches ~EncounterRun~, which
calls ~Encounter.run~ — bumps round to 1 and sets the active creature to the
queue head (highest initiative).  Once round >= 1 the same button dispatches
~NextTurn~ instead.
