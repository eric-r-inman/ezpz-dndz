...
- ~savedSnapshot : Maybe Encounter~ + ~savedAs : Maybe String~ — the most recent
  ...
  / Load success and consumed by the Encounter Saves panel, which marks its
  title when the live roster differs from the snapshot.
...
| ~condition_preset_sets~ → ~condition_presets~ | 0002, 0005, 0006 | Saved Add-Condition forms.                                        |
...
| ~encounters~ (+ creatures / conditions / save notices / recharge abilities / pips / treasure + its contributions / items) | 0004, 0005, 0006 | The live encounter, fully typed — treasure settings flatten onto the ~encounters~ row itself; absent row reads as ~null~. |
...
The editor column runs a small set of named flows that the rest of the app
routes through:
...
Compendium's group detail pane.  The text and the lore that justifies each
composition are collected in [[file:LORE_GROUPS.org][LORE_GROUPS.org]], which is where a new bundled
group's write-up goes.
