...
8. *Drawer registration* — a panel also needs a ~SurfaceLens~ in =Model.elm=, a
   ...
   ~Main.drawerEscSub~, and — for a panel the Actions column opens — an entry in
   ~Model.defaultDrawer~, the folded stack the drawer boots with; those four
   move together.  Panels live in the ~model.drawer~ stack — many open at once,
   newest at the bottom — and their Update modules open, close, and edit them
   through ~Model.openDrawer~ / ~closeDrawer~ / ~toggleDrawer~ / ~mapDrawer~.  A
   per-creature editor takes a narrower shape: its body lives in
