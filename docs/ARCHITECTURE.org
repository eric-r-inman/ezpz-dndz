...
— ~Model.PopupColor~), and sets ~Ui.Dice.rollBadgeOverride~ so the same three
totals, same colours, stay visible in the rail's badge strip next to the 🎲 icon
even after the popups fade.  Any single roll landing afterward — from any source
— clears the override via ~Effects.pushIncomingDiceRoll~, so the strip reverts
to its ordinary last-3-rolls-by-recency view until the next triple-roll.  No HP
mutation; the GM applies effects manually based on the result.
